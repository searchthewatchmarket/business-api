...
**Endpoint:** `https://data.searchthewatchmarket.com/mcp/http`  
...
2. Ask a question, e.g. _“What’s the market price for a Rolex Submariner 126610LN?”_
...
| **Days to sell**          | Typical 50% and 95% sell-within times for a model, in days                       |
...
{
  "mcpServers": {
    "Search the Watch Market Data": {
      "url": "https://data.searchthewatchmarket.com/mcp/http",
      "headers": {
...
2. Confirm **Search the Watch Market Data** appears and shows as connected.
...
| Setting           | Value                                            |
| ----------------- | ------------------------------------------------ |
| **Server URL**    | `https://data.searchthewatchmarket.com/mcp/http` |
| **Authorization** | `Bearer YOUR_API_KEY_HERE`                       |

Check your app’s documentation for where to add MCP servers. The name you choose (e.g. `Search the Watch Market Data`) is only a label for your own reference.
...
| Goal           | Example                                                                |
| -------------- | ---------------------------------------------------------------------- |
| Current price  | _What’s the current market price for a Rolex Submariner 126610LN?_     |
| Days to sell   | _How quickly does Omega Speedmaster 310.32.42.50.01.002 typically sell?_ |
| Specifications | _What are the specs for Omega Speedmaster 310.32.42.50.01.002?_        |
| Price trend    | _How has the Tudor Black Bay 58 price changed over the last 13 weeks?_ |
| New stock      | _Show me new listings for Rolex Datejust 126300 from the past week._   |
| Dealer pricing | _How has the asking price changed on listing 112578408?_               |
...
| Wrong URL                          | Use `https://data.searchthewatchmarket.com/mcp/http` — prefer `/mcp/http` over bare `/mcp` (proxies can break the handshake). Not a `/data/...` path. |
...
| Date       | Change                                       |
| ---------- | -------------------------------------------- |
| 2026-09-18 | Documented days-to-sell lookups              |
| 2026-07-13 | Prefer `/mcp/http` client URL; server label **Search the Watch Market Data** |
