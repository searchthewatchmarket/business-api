...
const UPSTREAM_BASE_URL = (
  process.env.UPSTREAM_BASE_URL || "https://data.searchthewatchmarket.com"
).replace(/\/$/, "");
...
// Demo UI calls production directly (see public/assets/app.js).
// This value is exposed so the browser uses the same host as UPSTREAM_BASE_URL.
app.get("/app/config.js", (req, res) => {
  res.setHeader("Content-Type", "application/javascript; charset=utf-8");
  res.setHeader("Cache-Control", "no-store");
  res.send(`window.DATA_API_BASE = ${JSON.stringify(UPSTREAM_BASE_URL)};\n`);
});

app.use("/app", express.static(path.join(__dirname, "public")));

// This process only serves the sample UI. It is not the Business API.
app.get("/", (req, res) => {
...
app.get("/data/model/:group_id/days-to-sell", requireDataAuth, (req, res) => {
  const groupId = parsePositiveInteger(req.params.group_id);
  if (!groupId) {
    return errorEnvelope(
      res,
  ...
  return proxyUpstreamGet(req, res, `/data/model/${groupId}/days-to-sell`);
});
...
app.listen(PORT, () => {
  console.log(`Example app UI: http://localhost:${PORT}/app`);
  console.log(`Business API (production): ${UPSTREAM_BASE_URL}`);
});