(function () {
  ...
  const els = {
    ...
    daysToSellCards: document.getElementById("daysToSellCards"),
    ...
    daysToSellPanel: document.getElementById("daysToSellPanel"),
    ...
    btnDaysToSell: document.getElementById("btnDaysToSell"),
    ...
    btnManualDaysToSell: document.getElementById("btnManualDaysToSell"),
  ...
  // Browser talks to production (or UPSTREAM_BASE_URL), not this local demo server.
  const API_BASE = String(
    window.DATA_API_BASE || "https://data.searchthewatchmarket.com",
  ).replace(/\/$/, "");
  ...
  function renderDaysToSell(data) {
    const metrics = [
      ["50% Sell Within", data.sell_within_50_days],
      ["95% Sell Within", data.sell_within_95_days],
    ];
    els.daysToSellCards.innerHTML = metrics
      .map(function (m) {
        const value =
          m[1] === null || m[1] === undefined ? "N/A" : `${m[1]} days`;
        return `<article class="metric"><div class="label">${m[0]}</div><div class="value">${value}</div></article>`;
      })
      .join("");
    setPanelVisible(els.daysToSellPanel, true);
  }
  ...
  async function fetchDaysToSell(groupId) {
    const result = await run(
      "GET /data/model/:group_id/days-to-sell",
      function () {
        return request(`/data/model/${groupId}/days-to-sell`, true);
      },
    );
    if (result && result.success) {
      renderDaysToSell(result.body);
    }
  }
  ...
  els.btnDaysToSell.addEventListener("click", function () {
    Promise.resolve()
      .then(getCurrentGroupIdOrThrow)
      .then(fetchDaysToSell)
      .catch(function (error) {
        renderClientError("GET /data/model/:group_id/days-to-sell", error);
      });
  });
  ...
  els.btnManualDaysToSell.addEventListener("click", function () {
    Promise.resolve()
      .then(function () {
        return getPositiveIntOrThrow(els.groupId, "group_id");
      })
      .then(fetchDaysToSell)
      .catch(function (error) {
        renderClientError("Manual GET days-to-sell", error);
      });
  });
