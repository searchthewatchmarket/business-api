...
- Retrieve 50% / 95% days-to-sell figures for a model
...
| GET /data/model/{group_id}/days-to-sell  | 50% / 95% sell-within times in days       |
...
Returns the two Model Stats sell-within figures for the model: how quickly listings for this reference typically leave the market.

These are the same two numbers shown on the website (50% sell within / 95% sell within).
...
```json
{
  "status": 1,
  "group_id": 2314,
  "sell_within_50_days": 29,
  "sell_within_95_days": 87
}
```

A field is omitted when that figure is not available for the model. If neither figure is available the endpoint returns 404.

Request example:

```bash
curl -s "https://data.searchthewatchmarket.com/data/model/2314/days-to-sell" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
### 6) Model Listings (New in Last 7 Days)
...
4. Use model endpoints for analytics (prices, days to sell, specs, history).
...
# Days to sell
curl -s "$BASE_URL/data/model/$GROUP_ID/days-to-sell" -H "Authorization: Bearer $API_KEY"
...
| Date       | Change                                                                      |
| ---------- | --------------------------------------------------------------------------- |
| 2026-09-18 | Added `GET /data/model/{group_id}/days-to-sell` (50% / 95% sell-within)     |
